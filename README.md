...
1. Rsync mmCIF files from the Protein Data Bank (PDB) and download mmCIF-associated FASTA files
   ...
   * Download the associated FASTA files
   ...
   Specifically, the ```scripts/run_update_fastas.py``` will parse the output of the SLURM job ```scripts/sbatch_rsync_mmcifs.sh``` to identify the PDB entries that need to be updated or are obsolete, and generates two corresponding list files, ```database/list_update_cifs.txt``` and ```database/list_obsolete_cifs.txt```.
...
1. Run SAAINT-parser to process all mmCIF files
   ```bash
    ...
    ```
   ```<mmCIF_path>``` represents a path to the downloaded mmCIF files, i.e., ```database/mmCIF_divided```.
