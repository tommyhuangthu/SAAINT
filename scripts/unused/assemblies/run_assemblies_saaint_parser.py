    ...
    if verbose:
        ...
        for ent in pdb_entities:
            for i, chain_id in enumerate(ent.get_chain_ids()):
                if ent.get_abrsa_type(i) == 'nonab': continue
        ...
        for ent_ag in pdb_entities:
            ...
            for ag_chain_id in ent_ag.get_chain_ids():
                ...
                if inf_res_num >= 10:
                    ...
                    # calculate the number of CDR residues on the interface
                    HL_cdr_inf_res = list(set(HL_cdr_all) & set(H_inf_res+L_inf_res))
                    HL_cdr_inf_res.sort()
                    if verbose: 
                        print(f'HL_cdr_inf_res: {HL_cdr_inf_res}')
                    cdr_inf_res_num = len(HL_cdr_inf_res)
                    cdr_inf_res_ratio = len(HL_cdr_inf_res)/len(H_inf_res+L_inf_res)

                    # calculate the number of FR residues on the interface
                    HL_fr_inf_res = list(set(H_inf_res+L_inf_res) - set(HL_cdr_all))
                    HL_fr_inf_res.sort()
                    fr_inf_res_num = len(HL_fr_inf_res)
                    if verbose:
                        print(f'HL_fr_inf_res: {HL_fr_inf_res}')
...
def parse_ab_ag_interaction(pdbid, pdb_fasta_path, pdb_cif_path):
    ...
    l2code = pdbid[1:3]
    pdb_fasta_file = f'{pdb_fasta_path}/{l2code}/{pdbid[0:4]}.fasta'
    ...
    for ent in pdb_entities:
        ...
        if verbose:
            print(f'chain_ids: {ent.get_chain_ids()}, fas_seq: {ent.get_fas_seq()}, fas_seq_len: {len(ent.get_fas_seq())}, abrsa_type (by fasta-seq): {abrsa_type}')
    ...
    model = structure[0]
    model_chain_ids, renamed_model_chain_ids = [], []
    for chain in model: 
        model_chain_ids.append(chain.get_id())
        renamed_model_chain_ids.append(re.sub(r'-[0-9]+', '', chain.get_id()))
    #exit(0)
    ...
    for id_ in model_chain_ids:
        if '-' in id_:
            chain_mapping[id_] = id_[0]
            single_letters.remove(id_[0])
    for id_ in model_chain_ids:
    ...
    if verbose:
        print('model_chain_ids:', model_chain_ids)
        print('renamed_model_chain_ids:', renamed_model_chain_ids)
    ...
    for ent in pdb_entities:
        new_ids = []
        for chain_id in ent.get_chain_ids():
            if chain_id in model_chain_ids:
                new_ids.append(chain_mapping[chain_id])
            elif chain_id in renamed_model_chain_ids:
                chain_id_index = renamed_model_chain_ids.index(chain_id)
                new_ids.append(chain_mapping[model_chain_ids[chain_id_index]])
                ...
                if chain_type == 'protein' or chain_type == 'peptide': 
                    ...
                    if abrsa_type == 'heavy_light':
                        ...
                        if tmscore < min_tmscore_ab_domain and ent.get_real_pdb_len(index) > min_len_ab_chain:
                            ent.set_abrsa_type(index, 'nonab')
                        if verbose:
                    ...
                    if verbose:
                        print(', '.join([f'protein chain_id: {chain_id}', f'filled-seq: {ent.get_filled_pdb_seq(index)}', f'filled_pdb_len: {ent.get_filled_pdb_len(index)}',
                            f'real_pdb_len: {ent.get_real_pdb_len(index)}', f'abrsa_type (by filled-seq): {abrsa_type}', f'mean_radius: {ent.get_mean_radius(index)}']))
    ...
    if all_paired_ab_ags:
        ...
        for chain_pair_line in chain_pair_lines:
            f_pairs.write(f'{chain_pair_line}')
        f_pairs.close()
...
if __name__ == '__main__':
    ...
    pdb_fasta_path = f'{abs_path}/../database/assemblies_fasta_divided'
    pdb_cif_path   = f'{abs_path}/../database/assemblies_mmCIF_divided'
